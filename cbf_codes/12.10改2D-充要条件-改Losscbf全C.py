...
def create_2d_simplex_from_data(data, n_vertices=200, c=0.1):
    # 球初始化
    """
    简化版本：从9维数据生成正单形平面参数
    """
    ...
    # 生成均匀球面点
    vertices = np.random.randn(n_vertices, 2)
    ...
    for i in range(w_vectors.shape[0]):
        ...
        w_vectors_3d.append([w1_3d, w2_3d])
        b_values_3d.append(b_3d)

    w_vectors_3d_tensor = torch.tensor(w_vectors_3d, dtype=torch.float32)
    b_values_3d_tensor = torch.tensor(b_values_3d, dtype=torch.float32)
    print(f"最终凸包边数: {n_vertices}")
    end_time = time.time()
    gouzaotime = end_time - start_time

    return w_vectors_3d_tensor, b_values_3d_tensor, gouzaotime
...
def matrix_wB(parameters):
    wB = []
    b = []
    B_tensor = torch.tensor(B, dtype=torch.float32)
    for wb_i in parameters:
        # 确保所有操作使用 torch 张量
        weight_i = wb_i['weight']
        bias_i = wb_i['bias']

        # 计算D矩阵元素
        d_i = torch.matmul(weight_i, B_tensor)
        wB.append(d_i)
        b.append(bias_i)
        # 计算R矩阵元素：

    # 将列表转换为张量
    wB = torch.stack(wB).reshape(num_networks, 2)
    b = torch.stack(b).reshape(num_networks, 1)
    return wB, b
...
def PlotDataGenerate(h_network):
    ...
    h_values_reshaped = h_values.detach().numpy().reshape(100, 100)
    return grid_points, h_values, grid_x, grid_y, h_values_reshaped


def plot2Dnew(h_network):
    """
    ...
    for fc in h_network.fc_list:
        w_i = fc.weight.detach().cpu().numpy().reshape(-1)  # (2,)
...
def plot_CIS(points, axis1, axis2):
    ...
    if points_projected.shape[0] >= 3:  # 确保有足够的点来计算凸包
        ...
        # 填充凸包内部并设置透明度
        plt.fill(points_projected[hull.vertices, 0], points_projected[hull.vertices, 1], '#ADD8E6', alpha=0.1,
                 label="Real_CIS")

        # 绘制凸包的边界
        for simplex in hull.simplices:
            plt.plot(points_projected[simplex, 0], points_projected[simplex, 1], color='#000000')

    plt.grid(True)
    plt.legend(loc='upper right')

def plot_CISpoints(data):
    arr = data[0]  # 取出数组
    N = arr.shape[0]  # 数组的行数就是点数 N
    print(N)
    print(f'data:{data}')
    points_array = data[0]


    plt.scatter(points_array[:, 0], points_array[:, 1], color='#00008B', alpha=0.7, label="match CBF condition")

def ploty(y_history):
    plt.figure()
    x = range(len(y_history))
    plt.plot(x, y_history, label='y_history')
    plt.title('plot of y_history')
    plt.xlabel('Index')
    plt.ylabel('y_label')
    plt.legend()
...
def plot_result(h_network, safe_points, unsafe_points,HARDCOUNT_SAFE, HARDCOUNT_UNSAFE,num_Xsafe,loss_history_safe, loss_history_unsafe, loss_history_cbf, loss_history_active, CBF_satisfied_coordinates):
    grid_points, h_values, grid_x, grid_y, h_values_reshaped = PlotDataGenerate(h_network)
    ...
    plot_CISpoints(CBF_satisfied_coordinates)

    # plot_safe_points(safe_points, 1, 2)
    # plot2D2(grid_points, h_values)
    plot2Dnew(h_network)
    # plot_index_x(CBF_index, safe_points, unsafe_points)
    plot3D(grid_x, grid_y, h_values_reshaped)
    # plotcommoncounts(Commom_counts)
...
def plot2D_cut_scatter(w_values, b_values):
    # 定义 x1 的范围
    x1_values = np.linspace(-1.5, 1.5, 100)  # 你可以根据需要调整范围和点的数量
    ...
    for w, b in zip(w_values, b_values):
        if np.abs(w[0]) >= 1e-6 and np.abs(w[1]) >= 1e-6:  # 通常情况，w1 和 w2 都不为零
            # 计算 x2 在 w1 x1 + w2 x2 + b = 0 时的值
            x2_values = -(w[0] / w[1]) * x1_values - (b / w[1])
            plt.scatter(x1_values, x2_values, label=f"w = {w}, b = {b}", s=10)
            print(0)
        elif np.abs(w[0]) <= 1e-6 and np.abs(w[1]) >= 1e-6:  # w1 = 0 且 w2 不为零
            # 计算 x2 在 w1 x1 + w2 x2 + b = 0 时的值
            x2_value = -b / w[1]  # x2 是常数
            plt.scatter(x1_values, np.full_like(x1_values, x2_value), label=f"w = {w}, b = {b}", s=10)
            print(1)
        elif np.abs(w[0]) >= 1e-6 and np.abs(w[1]) <= 1e-6:  # w1 不为零 且 w2 = 0
            # 计算 x1 在 w1 x1 + w2 x2 + b = 0 时的值
            x1_value = -b / w[0]  # x1 是常数
            x2_values = np.linspace(-1.5, 1.5, 100)  # x2 可以是任意值
            plt.scatter(np.full_like(x2_values, x1_value), x2_values, label=f"w = {w}, b = {b}", s=10)
            print(2)

        else:
                # 如果 b 不为零，方程没有解
                print(f"Warning: For w = {w}, b = {b}, no solution exists.")

    # 添加标签和图例
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title("Scatter Plot of the Line w1 x1 + w2 x2 + b = 0")
...
if __name__ == '__main__':
    ...
    # 统计满足CBF条件点的坐标
    CBF_index = []
    CIS_verticles = scipy.io.loadmat('CIS_vertices-2d.mat')['vertices']
    ...
    UNSAFE_NUM = []
    all_num = []
    safe_name = f'2D-safe-sample-10000.csv'
    unsafe_name = f'2D-unsafe-sample-20000.csv'
    # 数据集生成
    ...
    npoints = 20
    w_values, b_values, gouzaotime = create_2d_simplex_from_data(safe_points, npoints, 0.1)
    ...
    today = date.today()
    save_all_figures(f'C:/Users/XJ_lab/Desktop/实验仿真结果图/{today}-2D-Final/epsilon={epsilon}')
    ...
    # #  保存神经网络参数
    export_network_parameters(h_network, f'C:/Users/XJ_lab/Desktop/实验仿真结果图/{today}-2D-Final/epsilon={epsilon}'+'csv')
    print(f'2D优化问题求解平均时间：{sum(optimal_times)/len(optimal_times):.4f}')
    print(f'2D训练平均时间：{sum(train_times)/len(train_times):.4f}')
    print(f'2Ddeminate平均时间：{sum(kept_times)/len(kept_times):.4f}')
    print(f'构造凸包用时：{gouzaotime:.4f}\n')
    print(f'2D去冗余后平均点数:{kept_nall/len(kept_times):.2f}')
