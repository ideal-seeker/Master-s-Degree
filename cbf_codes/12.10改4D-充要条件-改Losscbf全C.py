...
def create_4d_simplex_from_data(data, n_vertices=200, c=0.1):
    """
    简化版本：从9维数据生成正单形平面参数
    """
    # 计算半径
    ...
    # 生成均匀球面点
    vertices = np.random.randn(n_vertices, 4)
    ...
    for i in range(w_vectors.shape[0]):
        ...
        w_vectors_5d.append([w1_5d, w2_5d, w3_5d, w4_5d])
        b_values_5d.append(b_5d)

    w_vectors_5d_tensor = torch.tensor(w_vectors_5d, dtype=torch.float32)
    b_values_5d_tensor = torch.tensor(b_values_5d, dtype=torch.float32)
    print(f"最终凸包边数: {n_vertices}")
    end_time = time.time()
    gouzaotime = end_time - start_time
    return w_vectors_5d_tensor, b_values_5d_tensor, gouzaotime
...
def compute_loss_N(h_network, x_safe, x_unsafe):
    ...
    #     xsafe=x_safe_tensor,
    #     params=params,
    #     num_per_piece=200//num_networks,  #
    #     random_seed=42
    # )
...
def PlotDataGenerate(h_network):
    ...
    grid_x1, grid_x2, grid_x3, grid_x4 = torch.meshgrid(x1_grid, x2_grid, x3_grid, x4_grid, indexing='ij')
    grid_points = torch.cat([grid_x1.reshape(-1, 1), grid_x2.reshape(-1, 1),  grid_x3.reshape(-1, 1), grid_x4.reshape(-1, 1)], dim=1)
    ...
    return grid_points, h_values, grid_x1, grid_x2, grid_x3, grid_x4
import torch
import numpy as np
import matplotlib.pyplot as plt
from itertools import combinations
...
def plot_CIS(points, axis1, axis2):
    ...
    if points_projected.shape[0] >= 3:  # 确保有足够的点来计算凸包
        ...
        # 绘制凸包的边界
        for simplex in hull.simplices:
            plt.plot(points_projected[simplex, 0], points_projected[simplex, 1], color='#000000', linewidth=2.0, solid_capstyle='round' )
...
def plot_result(h_network, safe_points, unsafe_points,HARDCOUNT_SAFE, HARDCOUNT_UNSAFE,num_Xsafe,loss_history_safe, loss_history_unsafe, loss_history_cbf, loss_history_active, CBF_satisfied_coordinates ):
    grid_points, h_values, grid_x1, grid_x2, grid_x3, grid_x4 = PlotDataGenerate(h_network)
    ...
    plot_CISpoints(CBF_satisfied_coordinates, 1, 2)
    # plot_safe_points(safe_points, 1, 2)
    # plot2D2(grid_points, h_values, 1, 2)
    plot2Dnew(h_network, 1, 2)


    plt.subplot(2, 3, 2)
    plot_CIS(CIS_verticles, 1, 3)
    plot_CISpoints(CBF_satisfied_coordinates, 1, 3)
    # plot_safe_points(safe_points, 1, 3)
    # plot2D2(grid_points, h_values, 1, 3)
    plot2Dnew(h_network, 1, 3)

    plt.subplot(2, 3, 3)
    plot_CIS(CIS_verticles, 1, 4)
    plot_CISpoints(CBF_satisfied_coordinates, 1, 4)
    # plot_safe_points(safe_points, 1, 4)
    # plot2D2(grid_points, h_values, 1, 4)
    plot2Dnew(h_network, 1, 4)

    plt.subplot(2, 3, 4)
    plot_CIS(CIS_verticles, 2, 3)
    plot_CISpoints(CBF_satisfied_coordinates, 2, 3)
    # plot_safe_points(safe_points, 2, 3)
    # plot2D2(grid_points, h_values, 2, 3)
    plot2Dnew(h_network, 2, 3)

    plt.subplot(2, 3, 5)
    plot_CIS(CIS_verticles, 2, 4)
    plot_CISpoints(CBF_satisfied_coordinates, 2, 4)
    # plot_safe_points(safe_points, 2, 4)
    # plot2D2(grid_points, h_values, 2, 4)
    plot2Dnew(h_network, 2, 4)

    plt.subplot(2, 3, 6)
    plot_CIS(CIS_verticles, 3, 4)
    plot_CISpoints(CBF_satisfied_coordinates, 3, 4)
    # plot_safe_points(safe_points, 3, 4)
    # plot2D2(grid_points, h_values, 3, 4)
    plot2Dnew(h_network, 3, 4)
...
def useless_lines(h_network):
    ...
    for i in range(len(B_all)):
        ...
        # 调用 linprog 求解线性规划问题
        result = linprog(c, A_ub=A, b_ub=b, bounds=[x1_bounds, x2_bounds, x3_bounds, x4_bounds], method='highs')
        if result.success:
...
if __name__ == '__main__':
    ...
    safe_name = f'4D-safe-sample-10000.csv'
    unsafe_name = f'4D-unsafe-sample-20000.csv'
    # 数据集生成
    ...
    npoints = 200
    w_values, b_values, gouzaotime = create_4d_simplex_from_data(safe_points, npoints, 0.1)
    ...
    today = date.today()
    save_all_figures(f'C:/Users/XJ_lab/Desktop/实验仿真结果图/{today}-4D-Final/epsilon={epsilon}')
    ...
    #  保存神经网络参数
    export_network_parameters(h_network, f'C:/Users/XJ_lab/Desktop/实验仿真结果图/{today}-4D-Final/epsilon={epsilon}'+'csv')
    print(f'4D优化问题求解平均时间：{sum(optimal_times)/len(optimal_times):.4f}')
    print(f'4D训练平均时间：{sum(train_times)/len(train_times):.4f}')
    print(f'4Ddeminate平均时间：{sum(kept_times)/len(kept_times):.4f}')
    print(f'4D构造凸包用时：{gouzaotime:.4f}\n')
    print(f'4D去冗余后平均点数:{kept_nall/len(kept_times):.2f}')
